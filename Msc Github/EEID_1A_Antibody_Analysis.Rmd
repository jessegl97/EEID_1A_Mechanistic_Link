...
```{r Source Data}
source("/Users/jesse/Documents/GitHub/EEID_1A_Mechanistic_Link/Reinfection_Augments_Heterogeneity/Public/R_Scripts/dataCleaning_EEID1A.R")
```
...
ab.all <- ggplot(data=p.abt, aes(x=dpi.f, y= elisa_od, 
                      color=fct_rev(primary_treatment)))+
  geom_hline(yintercept = 0.061, linetype="dashed", alpha=0.25)+
  geom_path(data=dat.new, position = position_dodge(width=-0.75), aes(x=dpi.f, y=yhat, color = fct_rev(primary_treatment), group=primary_treatment))+
...
```{r Proportional Variability Antibodies}
...
p.abi <- p.ab %>%
  filter(inf_prim == 1)%>%
  dplyr::select(dpi.f, primary_treatment, tes.new, elisa_od, quantity1)
...
ggplot(p.abi, aes(x=dpi.f, y=elisa_od))+
  geom_jitter(aes(color=fct_rev(primary_treatment)), alpha=0.5)+
  geom_point(data=a.cv, aes(x=dpi.f, y=mean_od, color=fct_rev(primary_treatment)))+
  geom_point(data=a.cv, aes(x=dpi.f, y=bird_pv, color=fct_rev(primary_treatment)), shape=5)+
  geom_path(data=a.cv, aes(x=dpi.f, y=bird_pv, color=fct_rev(primary_treatment)))

#Add pv values onto mean antibody levels
ggplot(data=p.abt, aes(x=dpi.f, y= elisa_od, 
                      color=fct_rev(primary_treatment)))+
  geom_hline(yintercept = 0.061, linetype="dashed", alpha=0.25)+
  geom_jitter(position = position_jitterdodge(dodge.width = 0.75, jitter.width=0.4), aes(group = fct_rev(primary_treatment)), size=2, alpha=0.5)+
      geom_jitter(data=dat.new, position = position_dodge(width=0.75), aes(x=dpi.f, y=yhat, group = fct_rev(primary_treatment)), color="black", size=5, alpha=1, shape="-", stroke=0.25)+
   geom_errorbar(data=dat.new,  position = position_dodge(width=0.75), aes(ymin=Lower, ymax = Upper, x=dpi.f, y=yhat, group=fct_rev(primary_treatment)), color="black", width=0.4)+  
geom_jitter(data=a.cv, position = position_dodge(width = -0.75), aes(x=dpi.f, y=bird_pv, group=primary_treatment), color='black', size=2, shape=0, stroke=1)+
  geom_jitter(data=a.cv, position = position_dodge(width = -0.75), aes(x=dpi.f, y=bird_pv, color=fct_rev(primary_treatment), group=primary_treatment), size=2, shape=15)+
  geom_path(data=a.cv, position = position_dodge(width = -0.75), aes(x=dpi.f, y=bird_pv, color=fct_rev(primary_treatment), group=primary_treatment))+
  #  geom_jitter(data=a.cv, position = position_dodge(width = -0.75), aes(x=dpi.f, y=bird_cv, group=primary_treatment), color='black', size=2, shape=2, stroke=1)+
  # geom_jitter(data=a.cv, position = position_dodge(width = -0.75), aes(x=dpi.f, y=bird_cv, color=fct_rev(primary_treatment), group=primary_treatment), size=2, shape=17)+
    labs(x= "Days Post Infection", color="Primary Treatment")+
  #facet_wrap(~sex)+
  theme_minimal()+
  scale_color_manual(values=c(pri_colors))+
    scale_y_continuous(
...
ggplot(data=p.abt, aes(x=dpi.f, y= elisa_od,
  ...
  geom_point(data=a.cv, aes(x=dpi.f, y=bird_pv, color=fct_rev(primary_treatment)), shape=5, size=2.5)+
  geom_path(data=a.cv, aes(x=dpi.f, y=bird_pv, group=primary_treatment, color=fct_rev(primary_treatment)), linetype="dotdash", size=0.5, alpha=0.5)+
    labs(y="Antibody Levels", 
       x= "Days Post Infection", shape="Primary Treatment", color="Primary Treatment")+
  scale_color_manual(values=c(pri_colors))+
  theme_minimal()+
...
```{r Max antibody level for each bird}

a.max <- p.abt %>%
  group_by(band_number)%>%
  mutate(max_od = max(elisa_od))

lm.ab.max <- glmmTMB(max_od~ primary_treatment, data=a.max, family=Gamma()) 
summary(lm.ab.max)
simulateResiduals(lm.ab.max, plot=T)
hist(resid(lm.ab.max))
car::Anova(lm.ab.max, type=3)


#specify model to predict
mod.pred <- lm.ab.max
#Model predictions
dat.new=expand.grid(primary_treatment=unique(a.max$primary_treatment),
                    max_od = unique(a.max$max_od))
                    #sex = unique(p.abt$sex))#new grid to put predictions into
dat.new$yhat = predict(mod.pred, type="response", newdata=dat.new, re.form=NA)
preds = predict(mod.pred, type="link", newdata=dat.new, se.fit=TRUE, re.form=NA)
dat.new = cbind(dat.new, preds)
...
a.cv.m <- a.max %>% 
  ...
  drop_na(max_od)%>%
  dplyr::summarise(
            group_cv = sd(elisa_od)/mean(elisa_od),
            group_sd = sd(elisa_od),
            group_pv = PV(elisa_od),
            max_od = max_od)
a.max.v <- left_join(a.max, a.cv.m)

ggplot(a.max.v)+
  geom_crossbar(data=dat.new, aes(ymin=Lower, ymax=Upper, x=primary_treatment, y=yhat,
                                  fill=fct_rev(primary_treatment)), alpha=0.01, width=0.5, stroke=0.01)+
  #geom_boxplot(aes(x=primary_treatment, y=max_od, fill=fct_rev(primary_treatment)),
  #            alpha=0.25, outlier.shape = NA)+
  geom_point(aes(x=primary_treatment, y=max_od, color=fct_rev(primary_treatment)),
              alpha=0.25, size=4)+
  geom_point(aes(x=primary_treatment, y=max_od),
              alpha=0.25, size=4, shape=1)+
  geom_point(data=a.max.v, aes(y=group_pv, x=primary_treatment, color=fct_rev(primary_treatment), fill=fct_rev(primary_treatment)), shape=25, size=4)+
  geom_point(data=a.max.v, aes(y=group_pv, x=primary_treatment), shape=25, size=4, color='black', alpha=0.01)+

    theme_minimal()+
  labs(x="Primary Treatment", y="Max Antibody OD", color="Primary Treatment", fill="Primary Treatment")+
    scale_y_continuous(
    sec.axi=sec_axis(~.*1, name="PV")
  )+
  theme_minimal()+
...
ggplot(p.abi, aes(x=dpi, y=elisa_od))+
  geom_jitter(aes(color=fct_rev(primary_treatment)), alpha=0.5)+
  geom_point(data=a.cv, aes(x=dpi, y=mean_od, color=fct_rev(primary_treatment)))+
  geom_point(data=a.cv, aes(x=dpi, y=bird_pv, color=fct_rev(primary_treatment)), shape=5)+
  geom_path(data=a.cv, aes(x=dpi, y=bird_pv, color=fct_rev(primary_treatment)))
...
ggplot(data=p.abt, aes(x=dpi.f, y= elisa_od, 
  ...
  geom_jitter(position = position_jitterdodge(dodge.width = 0.75, jitter.width=0.4), aes(group = fct_rev(primary_treatment)), size=2, alpha=0.5)+
      geom_jitter(data=dat.new, position = position_dodge(width=0.75), aes(x=dpi.f, y=yhat, group = fct_rev(primary_treatment)), color="black", size=5, alpha=1, shape="-", stroke=0.25)+
   geom_errorbar(data=dat.new,  position = position_dodge(width=0.75), aes(ymin=Lower, ymax = Upper, x=dpi, y=yhat, group=fct_rev(primary_treatment)), color="black", width=0.4)+  
 
  
ab.all+ geom_jitter(data=a.cv, position = position_dodge(width = -0.75), aes(x=dpi.f, y=bird_pv, group=primary_treatment), color='black', size=2, shape=0, stroke=1)+
  geom_jitter(data=a.cv, position = position_dodge(width = -0.75), aes(x=dpi.f, y=bird_pv, color=fct_rev(primary_treatment), group=primary_treatment), size=2, shape=15)+
  geom_path(data=a.cv, position = position_dodge(width = -0.75), aes(x=dpi.f, y=bird_pv, color=fct_rev(primary_treatment), group=primary_treatment))+
  #  geom_jitter(data=a.cv, position = position_dodge(width = -0.75), aes(x=dpi.f, y=bird_cv, group=primary_treatment), color='black', size=2, shape=2, stroke=1)+
  # geom_jitter(data=a.cv, position = position_dodge(width = -0.75), aes(x=dpi.f, y=bird_cv, color=fct_rev(primary_treatment), group=primary_treatment), size=2, shape=17)+
...
het.paper.only <- ggplot(het.df, aes(x=Metric))+
    geom_point(size=3,aes(x="CV", y=CV, color=dose), shape = 17)+
  geom_point(size=3.7,aes(x="CV", y=CV, shape = "CV"), color="black", shape=2, stroke = 1)+
  scale_color_manual(values=c(pri_colors))+
  scale_y_continuous(
...
ab.paper.only <- ggplot(het.df, x=Metric)+
  geom_point(size=4,aes(x="PV", y=ab.pv, color=dose, shape="PV"))+
  geom_point(size=4,aes(x="CV", y=ab.cv, color=dose, shape = "CV"))+
...
data <- data.frame(Dose = dose, 'Susceptibility_CV' = CV, `Antibody_PV` = ab.pv, `Antibody_CV` = ab.cv)
...
ggplot(data=p.abt, aes(x=dpi.f, y= elisa_od, 
  ...
  geom_hline(yintercept = 0.061, linetype="dashed", alpha=0.25)+
  geom_jitter(position = position_jitterdodge(dodge.width = 0.75, jitter.width=0.4), aes(group = fct_rev(primary_treatment)), size=2, alpha=0.5)+
      geom_jitter(data=dat.new.ab, position = position_dodge(width=0.75), aes(x=dpi.f, y=yhat, group = fct_rev(primary_treatment)), color="black", size=5, alpha=1, shape="-", stroke=0.25)+
   geom_errorbar(data=dat.new.ab,  position = position_dodge(width=0.75), aes(ymin=Lower, ymax = Upper, x=dpi.f, y=yhat, group=fct_rev(primary_treatment)), color="black", width=0.4)+  
  geom_jitter(data=a.cv, position = position_dodge(width = -0.75), aes(x=as.factor(dpi), y=bird_pv, group=primary_treatment), color='black', size=2, shape=0, stroke=1)+
  geom_jitter(data=a.cv, position = position_dodge(width = -0.75), aes(x=as.factor(dpi), y=bird_pv, color=fct_rev(primary_treatment), group=primary_treatment), size=2, shape=15)+
  geom_jitter(data=het.df, position = position_dodge(width = +0.75), aes(x=as.factor(dpi), y=mean_sus_inv_scaled, color=dose), size=2)+
  geom_jitter(data=het.df, position = position_dodge(width = +0.75), aes(x=as.factor(dpi), y=mean_sus_inv_scaled, groups=dose), color='red', shape= 1, size=2)+
  #  geom_jitter(data=a.cv, position = position_dodge(width = -0.75), aes(x=as.factor(dpi), y=bird_cv, group=primary_treatment), color='black', size=2, shape=2, stroke=1)+
  # geom_jitter(data=a.cv, position = position_dodge(width = -0.75), aes(x=as.factor(dpi), y=bird_cv, color=fct_rev(primary_treatment), group=primary_treatment), size=2, shape=17)+
    labs(x= "Days Post Infection", color="Primary Treatment")+
  #facet_wrap(~sex)+
  theme_minimal()+
  scale_color_manual(values=c(pri_colors))+
...
n.r <- m.ab %>%
  filter(band_number %in% c(2274, 2514, 2469, 2520, 2494))%>%
  dplyr::select(band_number, primary_treatment, dpi, secondary_dose, tes, quantity1)

ggplot(n.r, aes(x=dpi, y=log10(quantity1), color=as.factor(band_number)))+
  geom_path()+
  geom_path(aes(x=dpi, y=tes))+
  geom_hline(yintercept=log10(50))
...
band_number_summary <- m.ab %>%
  group_by(inf_sec, primary_treatment, secondary_treatment) %>%
  summarize(
    num_band_numbers = n_distinct(band_number)
  )
...
#difference <- setdiff(wibird, wibird1)


#could try scaling antibody levels to help w/ effect sizes: value - minimum / max - min 0 to 1
min_val <- min(wibird$elisa_od_14)
max_val <- max(wibird$elisa_od_14)

# Scale elisa_od_14 to a range of 0 to 1
wibird$scaled_elisa_od_14 <- (wibird$elisa_od_14 - min_val) / (max_val - min_val)
...
# Replace NA values in elisa_od_14 with corresponding values from elisa_od_41
# wibird <- wibird %>% 
#   mutate(elisa_od_14_new = ifelse(is.na(elisa_od_14), elisa_od_41, elisa_od_14))
...
glm.ab14.scaled <- glm(inf_sec ~ scaled_elisa_od_14 + log10.sec_dose, data=wibird, family=binomial())
#glm.ab14.new <- glm(inf_sec ~ elisa_od_14_new + log10.sec_dose, data=wibird, family=binomial()) #Interaction is not significant
...
dat.new=expand.grid(log10.sec_dose=unique(wibird$log10.sec_dose),
                    ...
                    #elisa_od_14_new=unique(wibird$elisa_od_14_new),
                    scaled_elisa_od_14 = unique(wibird$scaled_elisa_od_14))
...
#scaled
ggplot(wibird, aes(x=(scaled_elisa_od_14), y=(inf_sec), color=as.factor(log10.sec_dose)))+
...
ggplot(m.ab%>%filter(dpi==56), aes(x=as.factor(secondary_dose), y=elisa_od, color=as.factor(secondary_dose)))+
  ...
  scale_color_manual(values=sec_colors)+
  labs(title = "Antibody Levels Post-Secondary Infection", y= "Antibody Levels DPI 56", x="Secondary Dose", color="Secondary Dose", shape="Secondary Dose")+
  facet_grid(~inf_sec~primary_treatment)+
  theme_bw()

#ab levels dpi 
ggplot(m.ab%>%filter(dpi==41), aes(x=as.factor(secondary_dose), y=elisa_od, color=as.factor(secondary_dose)))+
  geom_hline(yintercept=0.061, linetype="dashed", alpha=0.5)+
  geom_boxplot(outlier.shape = NA)+
  geom_jitter(aes(color=as.factor(secondary_dose)), size=1.5, shape=1, alpha=0.75)+
  labs(x="Primary Treatment", y="ELISA OD", fill="Treatment")+
  stat_summary(aes(group=secondary_dose), fun=mean, shape = 18, size=.5)+
  scale_color_manual(values=sec_colors)+
  labs(title = "Antibody Levels Pre-Secondary Infection", y= "Antibody Levels DPI 41", x="Secondary Dose", color="Secondary Dose", shape="Secondary Dose")+
  facet_grid(~inf_sec~primary_treatment)+
  theme_bw()

#do antibody levels on day 14 predict antibody levels on day 56?
m.ab.w1 <- m.ab %>% 
  dplyr::select(dpi, band_number, primary_treatment, secondary_dose,
                          sex, elisa_od, inf_sec)%>%
  filter(dpi %in% c(-8, 14, 41, 56))

wibird1 <- m.ab.w1 %>% pivot_wider(names_from = dpi,
                                names_glue = "{.value}_{dpi}",
                  values_from = elisa_od)
wibird1 <- wibird1 %>%
  drop_na(elisa_od_14, elisa_od_41, elisa_od_56)
wibird1$sec_dose.n <- wibird1$secondary_dose+1
wibird1$log10.sec_dose <- round(log10(wibird1$sec_dose.n), digits = 2)


b<- ggplot(wibird1, aes(x=elisa_od_41, y=elisa_od_56, shape=primary_treatment, color=as.factor(secondary_dose)))+
  geom_point()+
  facet_grid(~inf_sec~primary_treatment)+
  theme_bw()

a <-ggplot(wibird1, aes(x=elisa_od_14, y=elisa_od_56, shape=primary_treatment, color=as.factor(secondary_dose)))+
  geom_point()+
  facet_grid(~inf_sec~primary_treatment)+
  theme_bw()

a+b
wibird1$inf_sec <- as.factor(wibird1$inf_sec)
lmx <- glmmTMB(elisa_od_56 ~ inf_sec + log10.sec_dose, data=wibird1, family=Gamma())
summary(lmx)

simulateResiduals(lmx, plot=T)

mod <- lmx
dat.new=expand.grid(#log10.sec_dose=unique(wibird1$log10.sec_dose),
                    inf_sec=unique(wibird1$inf_sec),
                    #elisa_od_14_new=unique(wibird$elisa_od_14_new),
                    elisa_od_56 = unique(wibird1$elisa_od_56),
                    log10.sec_dose = seq(min(wibird1$log10.sec_dose), max(wibird1$log10.sec_dose), length.out = 100))
                    #elisa_od_41 = unique(wibird$elisa_od_14))
dat.new$yhat=predict(mod, type="response", newdata = dat.new)
#prediction intervals
preds = predict(mod, type = "link", newdata = dat.new, se.fit =T)
#bind se's and fitted points
...
ggplot(wibird1, aes(x=log10.sec_dose, y=elisa_od_56))+
  geom_point(aes(color=fct_rev(inf_sec)))+
  geom_line(data=dat.new, aes(x=log10.sec_dose, y=yhat, color=fct_rev(inf_sec)))+
  geom_ribbon(data=dat.new, aes(x=log10.sec_dose, y=yhat, ymin=Lower, ymax=Upper, color=fct_rev(inf_sec)), alpha=0.25, lty="dashed")+
  facet_grid(~inf_sec)+
    theme_minimal()
...
#scale
min_val <- min(wibird$elisa_od_41)
max_val <- max(wibird$elisa_od_41)
wibird$scaled_elisa_od_41 <- (wibird$elisa_od_41 - min_val) / (max_val - min_val)
...
glm.ab41.scaled <- glm(inf_sec ~ scaled_elisa_od_41 + log10.sec_dose, data=wibird, family=binomial())
...
dat.new=expand.grid(log10.sec_dose=unique(wibird$log10.sec_dose),
                    inf_sec=unique(wibird$inf_sec),
                    #elisa_od_14_new=unique(wibird$elisa_od_14_new),
                    scaled_elisa_od_41 = unique(wibird$scaled_elisa_od_41))
                    #elisa_od_41 = unique(wibird$elisa_od_14))
dat.new$yhat=predict(mod, type="response", newdata = dat.new)
#prediction intervals
preds = predict(mod, type = "link", newdata = dat.new, se.fit =T)
...
#scaled
ggplot(wibird, aes(x=(scaled_elisa_od_41), y=(inf_sec), color=as.factor(log10.sec_dose)))+
...
```
```{r do baseline antibody levels predict susceptibility}
glm.ab.pre <- glm(inf_sec ~ `elisa_od_-8` + log10.sec_dose, data=wibird, family=binomial())
summary(glm.ab.pre)
...
#could try scaling antibody levels to help w/ effect sizes: value - minimum / max - min 0 to 1
min_val <- min(wibird$`elisa_od_-8`)
max_val <- max(wibird$`elisa_od_-8`)

# Scale elisa_od_14 to a range of 0 to 1
wibird$`scaled_elisa_od_-8` <- (wibird$`elisa_od_-8` - min_val) / (max_val - min_val)

glm.ab.pre.scaled <- glm(inf_sec ~ `scaled_elisa_od_-8` + log10.sec_dose, data=wibird, family=binomial())
...
dat.new=expand.grid(log10.sec_dose=unique(wibird$log10.sec_dose),
                    inf_sec=unique(wibird$inf_sec),
                    `elisa_od_-8`=unique(wibird$`elisa_od_-8`))
                    #`scaled_elisa_od_-8` = unique(wibird$`scaled_elisa_od_-8`))#new grid to put predictions into
dat.new$yhat=predict(mod, type="response", newdata = dat.new)
#prediction intervals
preds = predict(mod, type = "link", newdata = dat.new, se.fit =T)
#bind se's and fitted points
...
ggplot(wibird, aes(x=(`elisa_od_-8`), y=(inf_sec), color=as.factor(log10.sec_dose)))+
  geom_point(alpha=0.5)+
  geom_line(data=dat.new, aes(x=(elisa_od_.8), y=yhat), alpha = 1, size =1)+
   geom_ribbon(data = dat.new, aes(ymin=Lower, ymax = Upper, x= elisa_od_.8, y=yhat, fill=as.factor(log10.sec_dose)), alpha = 0.05, linetype="dashed", size=0.1) + 
  scale_color_manual(values = c(sec_colors))+
...
facet_wrap(~log10.sec_dose,labeller = as_labeller(sec_dose_names), nrow=1)+
  ...
  theme_minimal()+
    theme(axis.text.x = element_text(angle = 45, hjust = 1), 
        legend.position="top",
        legend.background= element_rect(size=0.25, linetype="solid"),
        legend.label = as_labeller(sec_dose_names))

ggsave('sus_ab_pre.jpeg', width=6, height=4, units='in')
```

```{r}
glm.ab.all <- glm(inf_sec ~ elisa_od + log10.sec_dose, data=m.ab %>% filter(dpi <42), family=binomial())
summary(glm.ab.all)
car::Anova(glm.ab.all, type=3)

dat.new=expand.grid(log10.sec_dose=unique(wibird$log10.sec_dose),
                    inf_sec=unique(wibird$inf_sec),
                    `elisa_od_-8`=unique(wibird$`elisa_od_-8`),
                    elisa_od_14=unique(wibird$elisa_od_14),
                    elisa_od_41=unique(wibird$elisa_od_41))
dat.new$yhat=predict(glm.ab.all, type="response", newdata = dat.new)
#prediction intervals
preds = predict(glm.ab.all, type = "link", newdata = dat.new, se.fit =T)
#bind se's and fitted points
dat.new = cbind(dat.new, preds)
#inverse link function
ilink <- family(glm.ab.all)$linkinv
#back transform CIs
dat.new <- transform(dat.new,
                     Fitted = ilink(fit),
                     Upper = ilink(fit + (2*se.fit)),
...
summary_table <- m.ab %>%
  filter(infected == 1 & dpi > 41) %>%
  group_by(dpi, primary_treatment, secondary_dose) %>%
  summarise(num_infected_birds = n_distinct(band_number)) %>%
  ungroup()

summary_table

e.pv.i <- left_join(e.pv.i, summary_table, by = c("dpi", "primary_treatment", "secondary_dose"))

#eye score PV infected birds
ggplot(e.pv.i%>% filter(dpi > 41), aes(x=dpi, y=bird_pv, color=fct_rev(primary_treatment)))+ 
  geom_point(aes(x=dpi, y=bird_pv, size=num_infected_birds), alpha=1)+
  #geom_point(aes(x=dpi, y=bird_cv), alpha=0.5)+
  scale_color_manual(values=pri_colors)+
  geom_path(aes(group=primary_treatment, x=dpi, y=bird_pv))+
    geom_text(aes(label = num_infected_birds, x = dpi, y = bird_pv), vjust = -0.75, size = 3, color='black') +
...
#remove uninfected birds ###This is sus I don't think it's correct b/c variability is too low with high secondary dose
p.pv.i <- m.ab %>%
  filter(dpi > 41 & inf_sec == 1) %>%
  group_by(dpi, primary_treatment, secondary_dose) %>%
  summarise(
...
p.pv.i <- left_join(p.pv.i, summary_table, by = c("dpi", "primary_treatment", "secondary_dose"))
...
ggplot(p.pv.i, aes(x=dpi, y=bird_pv, color=fct_rev(primary_treatment)))+ 
  ...
  geom_text(aes(label = num_infected_birds, x = dpi, y = bird_pv), vjust = -0.75, size = 3, color='black') +
  scale_color_manual(values=pri_colors)+
  geom_line(aes(group=primary_treatment))+
...
#path load quantity below cutoff removed by primary with model predictions
ggplot(p.i %>% filter(dpi > 41), aes(x=dpi.f, y=log10_quantity1, color=as.factor(log10.sec_dose)))+ 
  geom_jitter(size=2, alpha = 0.25, height = 0, width=0.15)+
  scale_color_manual(values=sec_colors[2:5])+
   stat_summary(aes(group=log10.sec_dose, color = as.factor(log10.sec_dose)), fun=mean, geom="line")+
  stat_summary(aes(group=log10.sec_dose, color = as.factor(log10.sec_dose)), fun=mean, geom="point")+
  # stat_summary(aes(group=primary_treatment), fun=mean, geom="point", color="black", shape=1)+
  #geom_point(data=dat.new, aes(x=dpi.f, y=yhat))+
  #geom_line()+
  #geom_line(data=dat.new, aes(x=dpi.f, y=yhat, group=log10.sec_dose))+
  #geom_errorbar(data=dat.new, aes(ymin=Lower, ymax=Upper, x=dpi.f, y=yhat), width=0.1, alpha=0.1)+
  labs(x="Day Post Infection", y= "Log10(Quantity+1)", color= "Primary Treatment", title = "Pathogen Load")+
