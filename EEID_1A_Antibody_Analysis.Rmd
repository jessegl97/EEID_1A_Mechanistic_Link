...
ggplot(m.ab%>% filter(dpi > 41), aes(x=dpi, y=tes, color=primary_treatment))+
  ...
  facet_wrap(~seropos_prim~primary_treatment)
...
lm2 <- glmmTMB(elisa_od~primary_dose + (1|band_number), data=p.ab, family=Gamma())
summary(lm2)
#model comparison 
...
ggplot(m.abs, aes(x=dpi, y=prop_inf, color=primary_treatment, shape=primary_treatment))+
  ...
  m.aball<- m.ab %>%
  group_by(dpi, primary_treatment, secondary_treatment)%>%
  summarise(prop_seropos = sum(seropos == 1, na.rm=TRUE) / n(),
            n_seropos = sum(seropos == 1, na.rm=TRUE),
            n_seroneg = sum(seropos == 0, na.rm=TRUE),
            prop_inf = sum(inf == 1, na.rm=TRUE)/ n(),
...
table <- m.aball %>%
  gt() %>%
  tab_header(
    title = "Summary of Bird Data",
    subtitle = "Grouped by DPI, Primary Treatment, and Secondary Treatment"
  ) %>%
  fmt_number(
    columns = vars(prop_seropos, prop_inf),
    decimals = 2
  ...
  cols_label(
    ...
    prop_seropos = "Proportion Seropositive",
    n_seropos = "Number of Seropositive Birds",
    n_seroneg = "Number of Seronegative Birds",
    prop_inf = "Proportion Infected",
    n_inf = "Number of Infected Birds",
    n_not_inf = "Number of Non-Infected Birds",
    seropos_prim = "Number of Seropositive Primary Infections",
    inf_prim = "Number of Infected Primary Infections",
...
table
#proportion infected
ggplot(m.aball, aes(x=dpi, y=prop_inf, color=primary_treatment, shape=primary_treatment))+
  geom_point(size = 1)+
  scale_color_manual(values = pri_colors)+
  geom_line()+
  ylim(c(0,1))+
...
```{r What portion of protection is from antibodies}
...
lm.sp1 <- glm(diseased_sec ~ seropos_prim + primary_treatment*secondary_dose, data=m.ab1, family=binomial(link= "logit"))
summary(lm.sp1)
...
dat.new=expand.grid(secondary_dose=unique(m.ab1$secondary_dose),
                    primary_treatment=unique(m.ab1$primary_treatment),
                    diseased_sec=unique(m.ab1$diseased_sec),
                    seropos_prim=unique(m.ab1$seropos_prim))#new grid to put predictions into
...
ggplot(m.ab1, aes(y=diseased_sec, x=seropos_prim))+
  ...
  labs(x="Seropositivity During Primary Infection", y="Disease During Secondary Infection", color = "Secondary Dose", shape = "Primary Treatment", linetype = "Primary Treatment")+
...
library(vcd)
#mosaic plot
mosaic(~primary_treatment + secondary_treatment + seropos_prim + diseased_sec, data=m.ab1,
       highlighting = "primary_treatment", highlighting_fill = pri_colors)

mplot<-mosaic(~primary_treatment + secondary_treatment + seropos_prim + diseased_sec, data=m.ab1,
       highlighting = "secondary_treatment", highlighting_fill = sec_colors)

```
```{r warning=FALSE}
plot <- m.ab %>%
  filter(dpi %in% c(-8, 14, 41, 56))
ggplot(plot, aes(y=elisa_od, x=dpi, color=fct_rev(primary_treatment)))+
  geom_point(shape = "-")+
  geom_line(aes(groups=as.factor(band_number)))+
  geom_hline(yintercept = 0.061)

#tes over all experiment
ggplot(m.ab, aes(x=dpi, y=tes, color=as.factor(secondary_dose)))+
  geom_point()+
  geom_hline(yintercept = 0.061)+
  stat_smooth(aes(x=dpi, y=tes, group=band_number), alpha=0.5, size=0.5, span=0.5, se=FALSE)+
  scale_color_manual(values = sec_colors)+
  labs(title = "TES - x = diseased sec, y = infected prim",x="Days Post Infection", y="TES", color="Secondary Dose")+
  facet_grid(~primary_treatment~diseased_sec~inf_prim)
...
ggplot(m.ab, aes(x=dpi, y=tes, color=as.factor(secondary_dose)))+
  ...
  stat_smooth(aes(x=dpi, y=tes, group=band_number), alpha=0.5, size=0.5, span=0.5, se=FALSE)+
  scale_color_manual(values = sec_colors)+
  labs(title = "TES",x="Days Post Infection", y="TES", color="Secondary Dose")+
  facet_grid(~secondary_dose~primary_treatment)

#path load over all experiment
ggplot(m.ab, aes(x=dpi, y=log10_quantity1, color=as.factor(secondary_dose)))+
  geom_point()+
  geom_hline(yintercept = 0.061)+
  stat_smooth(aes(x=dpi, y=log10_quantity1, group=band_number), alpha=0.5, size=0.5, span=0.5, se=FALSE)+
  scale_color_manual(values = sec_colors)+
  labs(title = "Path load - x = diseased sec, y = infected prim", x="Days Post Infection", y="Log10(quantity+1)", color="Secondary Dose")+
  facet_grid(~primary_treatment~diseased_sec~inf_prim)
...
ggplot(m.ab, aes(x=dpi, y=log10_quantity1, color=as.factor(secondary_dose)))+
  geom_point()+
  geom_hline(yintercept = 0.061)+
  stat_smooth(aes(x=dpi, y=tes, group=band_number), alpha=0.5, size=0.5, span=.75, se=FALSE)+
  scale_color_manual(values = sec_colors)+
  labs(title = "log10(quantity+1)",x="Days Post Infection", y="TES", color="Secondary Dose")+
  facet_grid(~secondary_dose~primary_treatment)
```
...
ggplot(m.abb %>% filter(dpi == 14), aes(x=primary_treatment, y=ab_change, color=fct_rev(primary_treatment)))+
  geom_jitter(width=0.25, height=0)+
  labs(x="Day Post Infection", y="Change in Antibody Levels", color="Primary Treatment", title = "Change in Antibody Levels: Day 14 - Baseline")
```
...
```{r}
bird.pv <- p.ab %>% 
  group_by(bird_ID, primary_treatment)%>%
  summarise(bird_cv = sd(elisa_od)/mean(elisa_od),
            bird_sd = sd(elisa_od),
            bird_pv = PV(elisa_od))
...
ggplot(s.cv%>%filter(dpi==56), aes(x=as.factor(secondary_dose), y=bird_cv, color=as.factor(secondary_dose)))+ 
  geom_point(size=2)+
  scale_color_manual(values=sec_colors)+
...
ggplot(s.cv%>%filter(dpi==56), aes(x=as.factor(secondary_dose), y=bird_pv, color=as.factor(secondary_dose)))+ 
  geom_point(size=2)+
  scale_color_manual(values=sec_colors)+
  #geom_path(aes(group=primary_treatment))+
