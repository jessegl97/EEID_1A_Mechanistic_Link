...
ggplot(data=p.abt, aes(x=dpi.f, y= elisa_od,
  ...
  geom_point(data=a.cv, aes(x=dpi.f, y=bird_pv, color=fct_rev(primary_treatment)), shape=5)+
  geom_path(data=a.cv, aes(x=dpi.f, y=bird_pv, group=primary_treatment, color=fct_rev(primary_treatment)), linetype="dotted", size=0.5, alpha=0.5)+
    labs(y="Antibody Levels", 
       x= "Days Post Infection", shape="Primary Treatment", color="Primary Treatment")+
  scale_color_manual(values=c(pri_colors))+
  theme_minimal()+
...
ggplot(a.max.v)+
  ...
  geom_point(data=a.max.v, aes(y=group_pv, x=primary_treatment, color=fct_rev(primary_treatment)), shape=25, size=4)+
  theme_minimal()+
...
het.paper.only <- ggplot(het.df, aes(x=Metric))+
  ...
  geom_point(size=3.7,aes(x="CoV", y=CoV, shape = "CoV"), color="black", shape=0, stroke = 2)+
  scale_color_manual(values=c(pri_colors))+
  scale_y_continuous(
...
#replacing NA with ELISA OD 41 Values
# ggplot(wibird, aes(x=(elisa_od_14_new), y=(inf_sec), color=as.factor(log10.sec_dose)))+
#   geom_line(data=dat.new, aes(x=(elisa_od_14_new), y=yhat), alpha = 1, size =1)+
#    geom_ribbon(data = dat.new, aes(ymin=Lower, ymax = Upper, x= elisa_od_14_new, y=yhat, fill=as.factor(log10.sec_dose)), alpha = 0.05, linetype="dashed", size=0.1) + 
...
dat.new=expand.grid(log10.sec_dose=unique(wibird$log10.sec_dose),
                    inf_sec=unique(wibird$inf_sec),
                    `elisa_od_-8`=unique(wibird$`elisa_od_-8`))#new grid to put predictions into
dat.new$yhat=predict(glm.ab.pre, type="response", newdata = dat.new)
#prediction intervals
preds = predict(glm.ab.pre, type = "link", newdata = dat.new, se.fit =T)
#bind se's and fitted points
dat.new = cbind(dat.new, preds)
#inverse link function
ilink <- family(glm.ab.pre)$linkinv
#back transform CIs
dat.new <- transform(dat.new,
                     Fitted = ilink(fit),
                     Upper = ilink(fit + (2*se.fit)),
