...
```{r Proportion Seroconverted day 14}
...
m.abs<- m.ab %>%
  ...
  summarise(prop_seropos = sum(seropos == 1, na.rm=TRUE) / n(),
            ...
            total_birds = sum(n_seropos, n_seroneg))
m.abs

ggplot(m.abs, aes(x=dpi, y=prop_seropos, color=primary_treatment, shape=primary_treatment))+
  geom_point(size = 3)+
  scale_color_manual(values = pri_colors)+
  labs(x = "Day Post Infection", y = "Proportion Seropositive", color = "Primary Treatment", shape = "Primary Treatment")#+
  guides(shape = FALSE)

ggplot(m.abs, aes(x = as.factor(dpi), y = prop_seropos, fill = fct_rev(primary_treatment))) +
  geom_bar(stat = "identity", position = "dodge") +
  labs(x = "Day post-infection (dpi)", y = "Proportion of Birds Seropositive") +
  theme_minimal()
