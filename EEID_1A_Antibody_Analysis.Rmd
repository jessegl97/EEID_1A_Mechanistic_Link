...
ggplot(m.abs, aes(x = as.factor(dpi), y = prop_seropos, fill = primary_treatment)) +
  geom_bar(stat = "identity", position = "dodge") +
  labs(x = "Day post-infection (dpi)", y = "Proportion of seropositive birds") +
  theme_minimal()

```
If birds seroconverted, were they protected from secondary pathology?
```{r}
#does seropositivity on day 14 after primary infection predict whether or not a bird will develop pathology upon secondary infection?
m.ab1 <- m.ab %>%
  group_by(band_number) %>%
  mutate(seropos_14 = ifelse(any(coalesce(dpi, 0) == 14 & coalesce(seropos, 0) == 1), 1, 0)) %>%
  ungroup()

lm.sp <- glm(diseased_sec ~ seropos_14 + primary_treatment*secondary_dose, data=m.ab1, family="binomial")

summary(lm.sp)
plot(allEffects(lm.sp))

resid <- simulateResiduals(lm.sp)
plot(resid)

lm.spa <- glm(diseased_sec ~ seropos_prim + primary_dose*secondary_dose, data=m.ab1, family="binomial")
summary(lm.spa)
plot(allEffects(lm.spa))
...
dat.new=expand.grid(secondary_dose=unique(m.ab1$secondary_dose),
                    ...
                    seropos_14=unique(m.ab1$seropos_14))#new grid to put predictions into
dat.new$yhat = predict(lm.sp, type="response", newdata=dat.new, re.form=NA) #predicted values
head(dat.new)

ggplot(m.ab1, aes(x=diseased_sec, y=as.factor(seropos_14)))+
  geom_count(aes(size=after_stat(n)))+
  geom_jitter(size=1, height=0)+
  geom_point(data = dat.new, aes(x = as.factor(seropos_14), y = yhat, color=as.factor(secondary_dose), shape=as.factor(secondary_dose))) +
  #geom_path(data = dat.new, aes(x = as.factor(seropos_14), y = yhat, color=as.factor(secondary_dose)))+
  labs(x="Disease During Secondary Infection", y="Antibodies During Primary Infection", color = "Secondary Dose", shape = "Secondary Dose")+
  facet_grid(~primary_treatment)




ggplot(m.ab1, aes(y=diseased_sec, x=as.factor(seropos_14)))+
...
ggplot(m.ab, aes(x=dpi, y=tes, color=as.factor(secondary_dose)))+
  geom_point()+
  geom_hline(yintercept = 0.061)+
  stat_smooth(aes(x=dpi, y=tes, group=band_number), alpha=0.5, size=0.5, span=1, se=FALSE)+
  scale_color_manual(values = sec_colors)+
  labs(x="Days Post Infection", y="TES", color="Secondary Dose")+
  facet_grid(~primary_treatment~diseased_sec)
