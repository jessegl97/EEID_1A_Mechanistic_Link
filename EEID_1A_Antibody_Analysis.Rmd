...
ggplot(not_recovered, aes(x=dpi, y=quantity, color=as.factor(band_number)))+
  geom_point()+
  ylim(min=0, max=400)
```
...
d.ab <- m.ab %>%
  filter(dpi < 42 & dpi > 0) %>%
  select(band_number, dpi, primary_treatment, sex, inf, quantity, diseased)
d.ab
...
glm.all <- glmmTMB(elisa_od~1 +(1|band_number), data=p.ab, family=Gamma())
glm.sham <- glmmTMB(elisa_od~1 +(1|band_number), data=subset(p.ab, primary_treatment == "sham"), family=Gamma())
glm.low <- glmemTMB(elisa_od~1 +(1|band_number), data=subset(p.ab, primary_treatment == "low"), family=Gamma())
glm.high <- glmmTMB(elisa_od~1 +(1|band_number), data=subset(p.ab, primary_treatment == "high"), family=Gamma())
...
```{r}
####Residual Models####
p.ab$band_number <- as.numeric(p.ab$band_number)
p.ab <- p.ab %>%
  filter(dpi <= 41 )%>%
...
ggplot(r.ab, aes(x=primary_treatment, y=abs(resid)))+
  geom_jitter()+
  theme_bw()
...
glm.all <- glmer(elisa_od~1 +(1|dpi), data=ab.1441, family=Gamma())
glm.sham <- glmer(elisa_od~1 +(1|dpi), data=subset(ab.1441, primary_treatment == "sham"), family=Gamma())
glm.low <- glmer(elisa_od~1 +(1|dpi), data=subset(ab.1441, primary_treatment == "low"), family=Gamma())
glm.high <- glmer(elisa_od~1 +(1|dpi), data=subset(ab.1441, primary_treatment == "high"), family=Gamma())
