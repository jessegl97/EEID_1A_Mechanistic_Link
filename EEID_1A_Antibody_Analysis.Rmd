...
aictab(cand.set=list(p1, p1.5, p1.75, p2, p3, p3.5, p4), modnames=c("p1", "p1.5", "p1.75", "p2", "p3", "p3.5", "p4"))

emm_results <- emmeans(lm1.5, ~ primary_treatment:dpi.f, scale="response")
pairs(emm_results)
summary(p1)
...
#antibody pv
pri.ab.var <- ggplot(a.cv , aes(x=as.factor(dpi), y=bird_pv))+
...
ab.pv <- c(0.04156784, 0.14716858, 0.18885516)
ab.cv <- c(0.03997555, 0.17202405, 0.26176249)
mean <- c(1.181, 0.446, 0.192)


het.df <- data.frame(dose, CoV, mean, ab.pv, ab.cv)

het.paper.only <- ggplot(het.df, aes(x= " "))+
    geom_point(size=3,aes(x="CoV", y=CoV, color=dose), shape = 19)+
  geom_point(size=3.7,aes(x="CoV", y=CoV, shape = "CoV"), color="red", shape=1, stroke = 2)+
  scale_color_manual(values=c(pri_colors))+
  scale_y_continuous(
...
ab.paper.only <- ggplot(het.df)+
  geom_point(size=3,aes(x="PV", y=ab.pv, color=dose, shape = "PV"), shape=15)+
   geom_point(size=3.6,aes(x="PV", y=ab.pv), color="black", shape=0, stroke = 2)+
  scale_color_manual(values=c(pri_colors))+
  scale_y_continuous(
...
het.paper <- ggplot(het.df)+
  geom_point(size=3,aes(x="PV", y=ab.pv*coeff, color=dose, shape = "PV"))+
   geom_point(size=3,aes(x="PV", y=ab.pv*coeff), color="black", shape=0, stroke = 2)+
  geom_point(size=3,aes(x="CV", y=ab.cv*coeff, color=dose, shape = "CV"))+
    geom_point(size=3,aes(x="CV", y=ab.cv*coeff), color="black", shape=2, stroke = 2)+
    geom_point(size=3,aes(x="CoV", y=CoV, color=dose, shape = "CoV"))+
  geom_point(size=3,aes(x="CoV", y=CoV, shape = "CoV"), color="red", shape=1, stroke = 2)+
  scale_color_manual(values=c(pri_colors))+
  scale_y_continuous(
    name= "Variability in Susceptibility (CoV) Hawley et al. 2024",
...
ggplot(wibird, aes(x=(elisa_od_14), y=(inf_sec), color=as.factor(log10.sec_dose)))+
  ...
  labs(x="Antibody Levels Day 14", y= "Susceptibility (Infected 1|0)", color="log10(Secondary Dose)", fill ="log10(Secondary Dose)")+
  ...
  #facet_wrap(~secondary_dose_fct, nrow=1, labeller = labeller(secondary_dose_fct = function(x) paste0("Log10(", x, "+1)")))
...
e.pv <- m.ab %>% 
  filter(dpi > 41)%>%
  group_by(dpi, primary_treatment, secondary_dose, inf_sec)%>%
  summarise(bird_cv = sd(tes.new)/mean(tes.new),
           bird_sd = sd(tes.new),
          bird_pv = PV(tes.new))
...
ggplot(e.pv.i, aes(x=dpi, y=bird_pv, color=fct_rev(primary_treatment)))+ 
  geom_point(size=2)+
  #geom_point(s.pv, aes(x=primary_treatment, y=bird_pv))+
  scale_color_manual(values=pri_colors)+
  geom_path(aes(group=primary_treatment))+
